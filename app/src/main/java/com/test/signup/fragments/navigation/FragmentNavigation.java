...
public class FragmentNavigation {
    ...
    private static final String WITH_TARGET_FRAGMENT_TAG_MARK = "FRAGMENT_WITH_TARGET";
    ...
    public boolean backTo(@NonNull final Function<FragmentManager.BackStackEntry, Boolean> condition) {
        ...
        for (int i = stackSize - 2; i >= 0; i--) {
            ...
            try {
                ...
                throw new UnexpectedException("we can't have exceptions in transactionSetup Function", exception);
            }
        }
        if (id != null && metCondition) {
    ...
    //ShortMethodName: it is ok because method's name is good!
    public boolean up() {
    ...
    /**
     * Backs to fragment with specific {@link #WITH_TARGET_FRAGMENT_TAG_MARK} tag.
     * This tag is adding if fragment added to stack via {@link #pushForResult} methods.
     *
     * @return True if it have back to target fragment in stack.
     */
    public boolean goToTarget() {
        return backTo(backStackEntry ->
                backStackEntry.getName() != null && backStackEntry.getName().endsWith(WITH_TARGET_FRAGMENT_TAG_MARK));
    }

    /**
     * Backs to fragment with specific {@link #WITH_TARGET_FRAGMENT_TAG_MARK} tag and after that execute method {@link #back()}
     * This tag is adding if fragment added to stack via {@link #pushForResult} methods.
     */
    public boolean goToTargetAndBack() {
        goToTarget();
        return back();
    }
    ...
    private boolean isFragmentTop() {
        final String topFragmentTag = getTopFragmentTag();
     ...
     */
    private void beforeSetInitialActions() {
    ...
    private void addToStack(@NonNull final Class<? extends Fragment> fragmentClass,
        ...
        if (targetFragment != null) {
            if (fragmentManager != targetFragment.getFragmentManager()) {
                throw new UnexpectedException("FragmentManager of target is differ then of creating fragment. "
                        + "Target will be lost after restoring activity. " + targetFragment.getFragmentManager() + " != " + fragmentManager);
            }
